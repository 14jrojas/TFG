...
def main():
    ...
    print('LISTA DE GPUS:',gpus)
    model = nn.DataParallel(model, device_ids=gpus).cuda()
    print("Finished constructing model!")

    # imprimir parámetros del modelo
    print(get_model_summary(model, torch.randn(1, 3, config.MODEL.IMAGE_SIZE[0], config.MODEL.IMAGE_SIZE[0])))
    ...
    for epoch in range(last_epoch, config.TRAIN.END_EPOCH):
        topk = (1,5) if dataset_name == 'imagenet' or dataset_name == 'birds' else (1,)
        if config.TRAIN.LR_SCHEDULER == 'step':
            lr_scheduler.step()
