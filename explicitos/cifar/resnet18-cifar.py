...
# logger
log_filename = datetime.now().strftime("output/resnet18-cifar_train_%Y_%m_%d_%H_%M.log")
...
train_loader = DataLoader(dataset=train_dataset, batch_size=96, shuffle=True, num_workers=2, pin_memory=True)
test_loader = DataLoader(dataset=test_dataset, batch_size=96, shuffle=False, num_workers=2, pin_memory=True)

# model
model = torch.hub.load('pytorch/vision:v0.9.0', 'resnet18', weights=None, num_classes=10)
...
lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer, len(train_loader)*220, eta_min=1e-6)
