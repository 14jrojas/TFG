...
# logger
log_filename = datetime.now().strftime("output/resnet50-birds_train_%Y_%m_%d_%H_%M.log")
...
train_loader = DataLoader(dataset=train_dataset, batch_size=32*2, shuffle=True, num_workers=6, pin_memory=True, drop_last=True)
test_loader = DataLoader(dataset=test_dataset, batch_size=32*2, shuffle=False, num_workers=6, pin_memory=True, drop_last=True)

# model
model = torch.hub.load('pytorch/vision:v0.9.0', 'resnet50', weights=None, num_classes=525)
